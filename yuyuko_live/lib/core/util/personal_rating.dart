...
import 'package:yuyuko_live/core/extension/calculation.dart';
import 'package:yuyuko_live/core/extension/number.dart';
...
class PersonalRating {
  static ShipRatingHolder rateSingle(Ratable? item) {
    ...
    final expected = GameRepository.instance.getPRData(item.shipID);
    if (expected == null || !expected.isAvailable)
      return ShipRatingHolder.unknown();

    final expectedDamage = expected.averageDamageDealt! * battles;
  ...
  static ShipRatingHolder rateMultiple(List<Ratable?> items) {
    ...
    for (final item in items) {
      ...
      final expected = GameRepository.instance.getPRData(item.shipID);
      if (expected == null || !expected.isAvailable) continue;
      ...
      expectedDamage += expected.averageDamageDealt! * battles;
